...
#curr_site <- "BARC"
  ...
  site_target <- targets_lm |>
    ...
    na.omit() #get rid of NAs to deal with NA issues later when you fit your model
...
#process uncertainty
 ...
 #change this to 
   # forecasted_temperature <- fit$coefficients[1] + (fit$coefficients[2] * site_drivers$air_temperature) + (fit$coefficients[3] * site_drivers$precipitation_flux)
forecasted_temperature <- param_df$beta1 + site_drivers$air_temperature * param_df$beta2 + site_drivers$precipitation_flux* param_df$beta3 + rnorm(n = n_members, mean = 0, sd = sigma) 
 ...
 # message(forecast_dates[t], ' date run')
}
  
  message(curr_site, ' forecast run') 
  }
  
  #to get a funnel shape must be driven by driver uncertainty
  #reducing initial uncertainty by adding a lag
```
...
```{r submit-forecast, eval=FALSE, include=FALSE}
