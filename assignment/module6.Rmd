...
**Answer Q.1**
Uncertainty is the set of possible outcomes based on the initial conditions, parameters, driver data, and process that encompasses the possible outcomes. 
...
**Answer Q.2**
Knowing the uncertainty allows managers to make more informed decisions. Incorporating uncertainty allows managers (And scientists) to make statements like "There is a ___% chance of this occurring" instead of being uncertain of the uncertainty and therefore unable to know the true likelihood of a given event occurring. 
...
Latitude:  29.675982 N
Longitude:  -82.008414 W
...
**Answer Q.4**
Increasing water temperature can cause more favorable conditions for phytoplankton blooms and for nutrient release. Basically, water temperature affects a whole host of variables and can also indicate when turnover is happening in a lake. 
...
**Answer Q.5**
Air temperature and water temperature appear to be fairly closely correlated, so I do think there is a relationship between air and water temperature. 
...
```{r}
airtemp_forecast <- plot_airtemp_forecast(lake_obs, weather_forecast, forecast_start_date)
airtemp_forecast
```
...
**Answer Q.6**
Each gray line represents an ensemble member, or a possible outcome based on the initial conditions. 
...
wtemp_yday is the water temperature yesterday
...
**Answer Q.7**
Water temperature is being predicted by yesterdays water temperature and the air temperature today (or the forecasted air temperature for t=t) 
...
**Answer Q.8**
Water temperature today has a much greater influence on forecasted water temperature than air temperature.

The standard errors for water temperature yesterday and the forecasted air temperature are similar (0.019 and 0.018) but air temperature se is a much greater proportion of air temperature's contribution. Therefore, I think we are more confident in the water temperature (which was measured) than the forecasted air temperature. 
...
About 90% of the variance is explained by the model, so the model fits the observations fairly well. The error and the rmse are fairly low, so the modeled values closely match the observed values. 
...
**Answer Q.10** Edit the code block to provide your answer. *16.2 degrees
```{r}
starting_temperature <- 15.2 #degrees C
...
**Answer Q.11** 
The water temperature mostly tracks the air temp forecast, but the water temp forecast increases rapidly between sep 25 and sep 27 and the air temp forecast doesn't increase as rapidly. However, because there are no new inputs for the water temperature, the forecast is driven by the air temperature forecast.  
...
**Answer Q.12**
The forecast uncertainty increases into the future. (there is a greater range of possible forecasted temperatures into the future.)
...
**Answer Q.13** 
The deterministic forecast implies that there will be one possible outcome, so I can either assume it is correct or assign my own uncertainty to it. The probabilistic forecast gives me a range of possible outcomes, and shows the uncertainty, so I know that the water temperature will likely fall within that range, and also that most likely it will be nearer the central tendency (but I don't like the spaghetti plots)
...
**Answer Q.14**
Parameter uncertainty is based on the parameters that inform the model. For this model, that is the coefficients for water temp and air temp. We are uncertain that the parameters are accurate, so we build in a distribution of possible values for the parameters. Having more data would reduce parameter uncertainty by improving the model fit.
...
Why do we only have driver data and param uncertainty in this model OR could we only have driver data without param uncertainty to do this???
...
**Answer Q.15**
Using an imprecise measurement for water temperature leads to IC uncertainty, so if the sensor is fouled or has not been calibrated well, then we cannot be certain that the temperature that it is reporting is accurate. 
...
for(i in 2:length(forecasted_dates)) {
  ...
  temp_pred$value <- param_df$beta1 + precip_driv$value * param_df$beta2 + temp_driv$value * param_df$beta3 + rnorm(n = 30, mean = 0, sd = sigma) 
...
**Answer Q.16**
The amount of uncertainty that is contributed is dependent on the time that you are looking at. Initially, initial conditions uncertainty is the greatest amount of uncertainty, after t=0, parameter uncertainty contributes the most of the total forecast uncertainty. 
...
**Answer Q.17**
different water temperature models will have slightly different quantities of uncertainty based on the amount of uncertainty for each type. driver uncertainty will be the same if the same drivers are used, but adjusting the model parameters and collecting more data could reduce uncertainty.
...
**Answer Q.19**
 Water temperature is being predicted by the water temperature at t-1, and the NOAA forecasts for air temperature and wind speed. This model also incorporates wind speed into the model, so there are 3 parameters.  
...
#run model - REVISE THIS!!!!!!
    temp_pred$value <- coeffs[1] + temp_lag$value * coeffs[2] + temp_driv$air_temperature * coeffs[3] + temp_driv$wnd *coeffs[4]
```
